...
class FakeLayerDownloader implements LayerDownloader {
  FakeLayerDownloader({this.results = const [], this.cancelError, Completer<void>? gateCompletion})
  ...
  Future<LayerDownloadResult> downloadLayer({
    ...
    if (preCancelInstanceId != null && preCancelStore != null) {
      preCancels.add(_PreCancel(id: preCancelInstanceId, storeName: preCancelStore.storeName));
    }
    calls.add(_Call(z: zoneUuid, sn: store.storeName, u: urlTemplate,
        zmin: minZoom, zmax: maxZoom, iid: instanceId));
    count++;
    onProgress(1.0);
  ...
  @override void pause(String z, FMTCStore s, String iid) {}
  @override void resume(String z, FMTCStore s, String iid) {}
}

class _Call {
  _Call({required this.z, required this.sn, required this.u, required this.zmin, required this.zmax, required this.iid});
...
void main() {
  ...
  setUp(() { repo = FakeMapLayerRepository(); });
  group('downloadZone — empty input', () {
    test('empty layers list returns without calling downloader', () async {
      ...
      final map = _m('empty'); repo.save(map);
  ...
  group('downloadZone — initial state', () {
    test('sets map status to downloading before completion', () async {
      ...
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('init-map'); repo.save(map);
    ...
    test('sets all layer statuses to downloading', () async {
      ...
      final d = FakeLayerDownloader(gateCompletion: gate);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('init-layers'); repo.save(map);
      final a = _l(LayerType.marine25k);
      final b = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, a); repo.saveLayer(map, b);
      unawaited(s2.downloadZone(map: map, layers: [a, b]));
      await Future.delayed(Duration.zero);
      expect(repo.findLayerById(a.id)!.downloadStatus, LayerDownloadStatus.downloading);
      expect(repo.findLayerById(b.id)!.downloadStatus, LayerDownloadStatus.downloading);
      gate.complete();
    });
  });
  ...
  group('downloadZone — completion states', () {
    test('all layers successful → map status ready', () async {
      final d = FakeLayerDownloader(results: [
        const LayerDownloadResult(downloadedTileCount: 100, estimatedTileCount: 100, successful: true),
        const LayerDownloadResult(downloadedTileCount: 50, estimatedTileCount: 50, successful: true),
      ]);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('all-ok'); repo.save(map);
      final a = _l(LayerType.marine25k); final b = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, a); repo.saveLayer(map, b);
      ...
      expect(repo.findLayerById(a.id)!.downloadStatus, LayerDownloadStatus.completed);
      expect(repo.findLayerById(b.id)!.downloadStatus, LayerDownloadStatus.completed);
    });
    test('all layers failed → map status failed', () async {
      final d = FakeLayerDownloader(results: [
        const LayerDownloadResult(downloadedTileCount: 0, estimatedTileCount: 100, successful: false),
        const LayerDownloadResult(downloadedTileCount: 0, estimatedTileCount: 50, successful: false),
      ]);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('all-fail'); repo.save(map);
      final a = _l(LayerType.marine25k); final b = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, a); repo.saveLayer(map, b);
      await s2.downloadZone(map: map, layers: [a, b]);
    ...
    test('mixed results → map status partial', () async {
      ...
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('partial'); repo.save(map);
      final a = _l(LayerType.marine25k); final b = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, a); repo.saveLayer(map, b);
      await s2.downloadZone(map: map, layers: [a, b]);
      final u = repo.findByUuid(map.uuid)!;
    ...
    test('successful layer records tile counts in fake repo', () async {
      final d = FakeLayerDownloader(results: [const LayerDownloadResult(downloadedTileCount: 42, estimatedTileCount: 100, successful: true)]);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('tile-count'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
  ...
  group('downloadZone — cancellation', () {
    test('cancelDownload sets map to failed', () async {
      ...
      final d = FakeLayerDownloader(gateCompletion: gate);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('cancel-fail'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
      unawaited(s2.downloadZone(map: map, layers: [layer]));
    ...
    test('cancelDownload delegates zone uuid to downloader', () async {
      ...
      final d = FakeLayerDownloader(gateCompletion: gate);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('cancel-delegate'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
  ...
  group('downloadZone — store routing', () {
    test('marine25k uses marine_zone_ uuid store', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('marine-store'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
      await s2.downloadZone(map: map, layers: [layer]);
      expect(d.calls.single.sn, 'marine_zone_${map.uuid}');
    });
    test('lidarLitto3d uses lidar_zone_ uuid store', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('lidar-store'); repo.save(map);
      final layer = _l(LayerType.lidarLitto3d); repo.saveLayer(map, layer);
      await s2.downloadZone(map: map, layers: [layer]);
      expect(d.calls.single.sn, 'lidar_zone_${map.uuid}');
    });
  });
  ...
  group('downloadZone — URL routing', () {
    test('marine10k URL contains RASTER_MARINE_10_WMTS_3857', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('url-10k'); repo.save(map);
      final layer = _l(LayerType.marine10k); repo.saveLayer(map, layer);
      await s2.downloadZone(map: map, layers: [layer]);
    ...
    test('marine50k URL contains RASTER_MARINE_50_WMTS_3857', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('url-50k'); repo.save(map);
      final layer = _l(LayerType.marine50k); repo.saveLayer(map, layer);
      await s2.downloadZone(map: map, layers: [layer]);
    ...
    test('zoom range forwarded from layer', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('zoom'); repo.save(map);
      final layer = _l(LayerType.marine25k);
      layer.minZoom = 12; layer.maxZoom = 18;
      repo.saveLayer(map, layer);
      await s2.downloadZone(map: map, layers: [layer]);
      expect(d.calls.single.zmin, 12); expect(d.calls.single.zmax, 18);
    });

    test('lidarLitto3d URL uses the regional layer covering the zone', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      ...
      final map = OfflineMap.create(
        uuid: 'bzh', name: 'Bretagne sud',
        northLat: 47.8, southLat: 47.0,
        westLng: -3.5, eastLng: -2.0,
      );
      repo.save(map);
      final layer = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, layer);
      final bounds = LatLngBounds(
        LatLng(47.0, -3.5), LatLng(47.8, -2.0),
      );
      await s2.downloadZone(map: map, layers: [layer], zoneBounds: bounds);
      // URL must NOT be the buggy "first layer" (Normandie).
      expect(d.calls.single.u, isNot(contains('L3D_MAR_NHDF_2016_2018_PYR_3857_WMTS')));
    ...
    test('lidarLitto3d URL falls back to first layer when bounds outside any region', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      // Middle of the Atlantic — no LiDAR region covers this.
      final map = OfflineMap.create(
        uuid: 'ocean', name: 'Ocean',
        northLat: 30.0, southLat: 25.0,
        westLng: -40.0, eastLng: -30.0,
      );
      repo.save(map);
      final layer = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, layer);
      final bounds = LatLngBounds(
        LatLng(25.0, -40.0), LatLng(30.0, -30.0),
      );
      await s2.downloadZone(map: map, layers: [layer], zoneBounds: bounds);
      // Falls back to first layer (no candidate for the bounds).
      expect(d.calls.single.u, contains('L3D_MAR_NHDF_2016_2018_PYR_3857_WMTS'));
    });

    test('lidarLitto3d URL with null zoneBounds uses first layer', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('lidar-null'); repo.save(map);
      final layer = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, layer);
    ...
    test('lidarLitto3d Occitanie — initial creation and update select the same regional dataset', () async {
      ...
      final map1 = OfflineMap.create(
        uuid: 'occ-1', name: 'Occitanie initial',
        northLat: 44.0, southLat: 43.0,
        westLng: 2.5, eastLng: 3.5,
      );
      repo.save(map1);
      final layer1 = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map1, layer1);
      final occitanieBounds = LatLngBounds(
        LatLng(43.0, 2.5), LatLng(44.0, 3.5),
      );
      await s1.downloadZone(map: map1, layers: [layer1], zoneBounds: occitanieBounds);
      final urlInitial = d1.calls.single.u;
      expect(urlInitial, isNot(contains('NHDF')),
          reason: 'Initial creation must use an Occitanie dataset, not NHDF. '
              'Got: $urlInitial');
      expect(
      ...
      final map2 = OfflineMap.create(
        uuid: 'occ-2', name: 'Occitanie update',
        northLat: 44.0, southLat: 43.0,
        westLng: 2.5, eastLng: 3.5,
      );
      repo.save(map2);
      ...
      await s2.downloadZone(map: map2, layers: [layer2],
          zoneBounds: reconstructedBounds);
      final urlUpdate = d2.calls.single.u;
      expect(urlUpdate, isNot(contains('NHDF')),
          reason: 'Update must use an Occitanie dataset, not NHDF. Got: $urlUpdate');
      expect(urlUpdate, equals(urlInitial),
          reason: 'Update must select the same Occitanie dataset as initial. '
  ...
  group('onProgress callback', () {
    test('receives layer labels for each layer', () async {
      final d = FakeLayerDownloader(results: [
        const LayerDownloadResult(downloadedTileCount: 10, estimatedTileCount: 10, successful: true),
        const LayerDownloadResult(downloadedTileCount: 5, estimatedTileCount: 5, successful: true),
      ]);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('progress'); repo.save(map);
      final a = _l(LayerType.marine25k); final b = _l(LayerType.lidarLitto3d);
      repo.saveLayer(map, a); repo.saveLayer(map, b);
      final labels = <String>[];
      await s2.downloadZone(map: map, layers: [a, b],
        onProgress: ({required progress, required layerLabel}) { labels.add(layerLabel); });
  ...
  group('isDownloading / activeCount', () {
    test('isDownloading true during active download', () async {
      ...
      final d = FakeLayerDownloader(gateCompletion: gate);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('is-dl'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
    ...
    test('activeCount is 1 during download', () async {
      ...
      final d = FakeLayerDownloader(gateCompletion: gate);
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('active'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
  ...
  group('downloadZone — FMTC instance ID hygiene (regression for "Bad state: A download instance with ID ... already exists")', () {
    test('two consecutive runs of the same zone use distinct instance ids', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('inst-id'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
      ...
      expect(d.calls[0].iid, isNot(d.calls[1].iid),
          reason: 'Two runs of the same zone must use different FMTC '
              'instance ids to avoid "A download instance with ID ... '
              'already exists".');
    });

    test('preCancelInstanceId is forwarded to the downloader as the previous run id', () async {
      final d = FakeLayerDownloader();
      final s2 = ZoneDownloadService(repository: repo, downloader: d);
      final map = _m('pre-cancel'); repo.save(map);
      final layer = _l(LayerType.marine25k); repo.saveLayer(map, layer);
      ...
      expect(d.preCancels, isNotEmpty,
          reason: 'downloadZone must forward a pre-cancel id so FMTC cleans '
